...
| **Shot cards** | One card per segment: a still of the frame at the in-point (a few KB, not a stream — see the latency section), the transcript lines that fall inside the cut, why it was chosen (editable), trim controls, drag to reorder. Trim the in-point and the still follows once the trimming settles |
...
| **Add a moment** | Three tabs. **kept** — the bin, first and the one the board opens on when the pass has kept anything: one row per keep, heroes first then by clip and start, with a still at its start, `CLIP_04 · 3:44.0 → 3:48.2 · 4.2 s`, `★ HERO`, the pass's reason and the editor's note, and either *in the cut · shot N* (click selects the shot) or *+ add to cut*; a keep whose footage left says so and cannot be added; an empty bin points at the pass. The Project panel carries one line from the same read — `bin · 7 moments · 2 heroes · 1:12 if strung out` — and the *first cut* step says `N heroes waiting` while a hero is not in the cut. Re-read when the tab is shown and after every save while it is up. **heard** — audio candidates not already in the cut, ranked — and **seen** — what the visual pass found, ranked by the bin's `events.json` (kind × corroboration from the motion and onset tracks × a closer look's confirmation), with unconfirmed claims sealed as such. One click to insert |
